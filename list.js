...
window.addEventListener("load", () => {
  ...
  filterListByUser.map((item) => {
    list_card_wrapper.innerHTML += `
      <div
          class="w-full p-4 flex flex-col gap-2 bg-white shadow-md rounded-lg"
        >
          ...
          <ul class="w-full flex flex-col gap-2">
            ${item.list
              .map((e) => {
                return `
                <li
              class="h-12 flex items-center px-2 border-b-[1px] border-orange-500"
            >
              ${e.name}
          ...
          <button
            class="h-12 w-[120px] ml-auto bg-orange-500 text-white flex justify-center items-center rounded-md mt-4" onclick="handleFinish(event)" 
...
const handleFinish = (event) => {
  ...
  //filter _liatbelanja sesuai id ;ist_id
  ...
  localStorage.setItem(
    "_listbelanja_finish",
    JSON.stringify(_listbelanja_finish)
  );
